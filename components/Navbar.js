...
export const Navbar = ({ appState, updateState, changePage, buildNewCharacterModal }) => {
  ...
  const selectedCharacter = appState.savedCharacters.find((character) => character.id === appState.character);
  ...
  return (
    <header>
      ...
      <motion.nav className={charactersSaved ? (navOpen ? "nav open" : "nav closed") : undefined} key="nav">
        <div className="container">
          {charactersSaved && (
            <motion.div
              ...
              links.map((link, index) => {
                ...
                return (
                  ...
                  >
                    <Link
                      href={`${path}`}
                      className={`playerclass-${selectedCharacter.playerClass}${isActive ? " active" : ""}`}
                      onClick={() => {
                        changePage(path);
                        windowWidth < 992 && toggleNav(false);
                      }}
