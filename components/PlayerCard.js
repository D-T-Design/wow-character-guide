...
const PlayerCard = ({ savedCharacters, currentCharacter, selectCharacter }) => {
  ...
  const clickHandler = useClickOutside([characterListRef, toggleButtonRef], toggleOpen, toggleCharacterList);
  ...
  return (
    <figure className={`playercard ${playerClass.toLowerCase()}`}>
      ...
      >
        ...
        <img src={`/static/img/faction/${faction.toLowerCase()}.png`} title={faction} alt={faction} />
        <img
          src={`/static/img/race/${race.toLowerCase().replace(/\s/g, "")}-${gender ? gender : "male"}.png`}
          title={race}
          alt={race}
        />
        <img
